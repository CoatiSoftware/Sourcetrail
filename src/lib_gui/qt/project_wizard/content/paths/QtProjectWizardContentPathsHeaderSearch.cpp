...
QtProjectWizardContentPathsHeaderSearch::QtProjectWizardContentPathsHeaderSearch(
	...
	, m_showDetectedIncludesResultFunctor(std::bind(
		&QtProjectWizardContentPathsHeaderSearch::showDetectedIncludesResult, this, std::placeholders::_1))
...
void QtProjectWizardContentPathsHeaderSearch::populate(QGridLayout* layout, int& row)
{
	...
	if (!m_indicateAsAdditional)
	{
		...
		}
		{
			...
			connect(validateionButton, &QPushButton::clicked, this, &QtProjectWizardContentPathsHeaderSearch::validateIncludesButtonClicked);
			layout->addWidget(validateionButton, row, QtProjectWizardWindow::BACK_COL, Qt::AlignRight | Qt::AlignTop);
		}
		row++;
	}
}
...
void QtProjectWizardContentPathsHeaderSearch::detectIncludesButtonClicked()
{
	...
	connect(m_pathsDialog.get(), &QtPathListDialog::finished, this, &QtProjectWizardContentPathsHeaderSearch::finishedSelectDetectIncludesRootPathsDialog);
...
void QtProjectWizardContentPathsHeaderSearch::showDetectedIncludesResult(const std::set<FilePath>& detectedHeaderSearchPaths)
{
	...
	else
	{
		...
		connect(m_filesDialog.get(), &QtTextEditDialog::finished, this, &QtProjectWizardContentPathsHeaderSearch::finishedAcceptDetectedIncludePathsDialog);
...
void QtProjectWizardContentPathsHeaderSearch::showValidationResult(const std::vector<IncludeDirective>& unresolvedIncludes)
{
	...
	else
	{
		...
		connect(m_filesDialog.get(), &QtTextEditDialog::finished, this, &QtProjectWizardContentPathsHeaderSearch::closedFilesDialog);
		connect(m_filesDialog.get(), &QtTextEditDialog::canceled, this, &QtProjectWizardContentPathsHeaderSearch::closedFilesDialog);
	}
}
