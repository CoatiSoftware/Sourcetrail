...
private:
	inline void checkIndexInRange(
		unsigned int columnIndex, unsigned int rowIndex, const std::string& function) const
	{
		std::stringstream message;

		if (columnIndex >= N)
		{
			message << function << ": columnIndex " << columnIndex
					<< " is out of range, maximum is " << N - 1;
		}
		...
		if (rowIndex >= M)
		{
			...
			message << function << ": rowIndex " << rowIndex << " is out of range, maximum is "
					<< M - 1;
		}
...
template <class T, unsigned int N, unsigned int M>
void MatrixBase<T, N, M>::setValue(
