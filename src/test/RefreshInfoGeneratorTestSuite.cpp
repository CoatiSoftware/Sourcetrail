...
TEST_CASE("nonindexed unchanged headerfile that is nottoindex") // this test does not make much sense without source files
{
...
TEST_CASE("nonindexed changed headerfile that is nottoindex")
{
	...
	REQUIRE(REFRESH_UPDATED_FILES == refreshInfo.mode);
	REQUIRE(1 == refreshInfo.nonIndexedFilesToClear.size());; // must be cleard here and will be re-indexed on demand
	REQUIRE(0 == refreshInfo.filesToClear.size());
	REQUIRE(0 == refreshInfo.filesToIndex.size());
}
...
TEST_CASE("indexed unchanged headerfile that is nottoindex") // TODO: check if depending source file gets reindexed
{
...
TEST_CASE("refresh info for updated files clears and reindexes known outdated file and referencing source file")
{
	cleanup();
	{
		const FilePath upToDateSourceFilePath = m_sourceFolder.getConcatenated(L"up_to_date_file.cpp");
...
TEST_CASE("refresh info for updated files clears known outdated header file and reindexes referencing source file")
{
	cleanup();
	{
		const FilePath upToDateSourceFilePath = m_sourceFolder.getConcatenated(L"up_to_date_file.cpp");
...
TEST_CASE("refresh info for updated files clears unknown outdated header file and reindexes referencing source")
{
	cleanup();
	{
		const FilePath upToDateSourceFilePath = m_sourceFolder.getConcatenated(L"up_to_date_file.cpp");
...
TEST_CASE("refresh info for updated files clears outdated source file and referened uptodate header file")
{
	cleanup();
	{
		const FilePath outdatedSourceFilePath = m_sourceFolder.getConcatenated(L"outdated_file.cpp");
